...
ggmap.xfoot.obs <- function(mm, lon.lat, site, oco2.path, facet.nrow = NULL,
  facet.ncol = NULL, nhrs.back, dpar, stilt.ver, foot.str = NULL, timestr,
  font.size = rel(0.9), recp.lon, recp.lat, foot, picname, storeTF,
  width = 12, height = 8){

  col <- def.col()
  m1 <- mm[[1]] + theme_bw() + coord_equal()
  ...
  title <- paste0('Spatial time-integrated weighted column footprint (',
    nhrs.back, ' hours back; ', dpar, ' dpar; ', foot.str,
    ') for overpass on ', timestr, ' for ', site,
    '\nSmall footprints < 1E-6 are displayed in gray')
    #'\nTotal footprint in plotted domain:', signif(sum.foot, 3), 'umol/m2/s')
