...
ggmap.xco2.obs <- function(mm, lon.lat, site, facet.nrow, nhrs.back, dpar, sf, 
  ...
  title <- paste('Spatial contribution of XCO2 [ppm] (',
    ...
    ')\nusing STILT version', stilt.ver, 'for overpass on', timestr, 'over',
  ...
  p2 <- p1 +
    ...
    scale_fill_gradientn(limits = c(1E-6, max(sel.xco2$xco2)),
      name = 'XCO2 [ppm]', colours = col, trans = 'log10',
  ...
  p3 <- p2 + theme(legend.position = 'bottom',
    ...
    legend.key = element_blank(), legend.key.width = unit(width/10, 'cm'),
    legend.key.height = unit(height/30, 'cm'),
  ...
  p4 <- p3 + scale_colour_gradientn(name = 'XCO2:', colours = col,
